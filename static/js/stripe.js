...
async function createStripeSession() {
 ...
 switchPaymentMethod('stripe', '')
  ...
  try {
    ...
    const { client_secret } = data; //Django returns the client secret
...
async function _stripeFormSubmit(e) {
  ...
  const { error } = await stripe.confirmPayment({//sends the card data directly to stripe
    elements, 
    confirmParams: {
...
async function _stripeInit() {
    const { data } = await axios("/checkout/stripe/config"); //asks for the public key from the views.py
    stripe = Stripe(data.public_key, { locale: 'ar' });
    _checkStripePaymentStatus();//checks if the user just returned from a payment to show a success/fail message
}
